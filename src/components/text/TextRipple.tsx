...
function CustomTextComponent({ text, position }) {
  const ref = useRef();
  const matRef = useRef();
  ...
  return (
    <>
      ...
      <mesh
        ref={ref}
        position={position}
        onPointerMove={(e) => (mouse.current = e.uv)}
      >
        <planeGeometry attach="geometry" args={[40, 40, 64, 64]} />
...
function TextRippleComponent({ text }) {
  return (
    <Canvas style={{ width: "100%" }}>
      ...
      <CustomTextComponent text={text} position={[0, 0, 0]} />
    </Canvas>
  );
}
